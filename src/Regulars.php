...
class Regulars {
    ...
    public function isIdCard($id_card){
        ...
        strlen($id_card) == 15 && $id_card=$this->idcard_15to18($id_card);
    ...
    // 将15位身份证升级到18位
    private function idcard_15to18($id_card){
        ...
        return $idcard . ($this->idcard_verify_number($idcard));
    }

    // 18位身份证校验码有效性检查
    private function idcard_checksum18($id_card){
        if(strlen($id_card) != 18){
            return false;
        }
        $idcard_base=substr($id_card,0,17);
        return $this->idcard_verify_number($idcard_base) == strtoupper(substr($id_card,17,1));
    }
